...
{
    public class UserService(IUnitOfWork unitOfWork, IPasswordHasher passwordHasher, IMapper mapper, IValidator<UserCreateDto> userValidator, IValidator<UserProfileUpdateDto> profileValidator) : IUserService
    {
        /// <summary>
        ...
        public async Task<OperationResult> UpsertUserAsync(UserCreateDto dto)
        {
            ...
            if (dto.Id == 0)
            {
                if (dto.RoleId == administratorRoleId)
                {
                    var allUsers = await unitOfWork.Users.GetAllAsync();
                    
                    bool administratorExists = allUsers.Any(u => u.RoleId == administratorRoleId && !u.IsDeleted);
                ...
                var existingUser = await unitOfWork.Users.GetByUsernameAsync(dto.Username.Trim());
            ...
            else
            {
                ...
                bool isProtectedAdministrator = user.Username.Equals("admin", StringComparison.OrdinalIgnoreCase) || user.RoleId == administratorRoleId;

                if (isProtectedAdministrator && dto.RoleId != administratorRoleId)
                ...
                if (!isProtectedAdministrator && dto.RoleId == administratorRoleId)
                {
                    ...
                    bool administratorExists = allUsers.Any(u => u.RoleId == administratorRoleId && u.Id != dto.Id && !u.IsDeleted);
        ...
        public async Task<OperationResult> UpdateProfileAsync(UserProfileUpdateDto dto)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dto.NewPassword))
            {
                ...
                bool isCurrentPasswordValid = passwordHasher.Verify(dto.CurrentPassword, user.Password);
        ...
        public async Task<OperationResult> ToggleUserStatusAsync(int id)
        {
            ...
            string accountStatus = user.IsActive ? "activada" : "desactivada";

            return operationSucceeded
                ? OperationResult.Ok($"La cuenta de usuario ha sido {accountStatus} exitosamente.")
